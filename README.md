<div align="center">
  <h1 align="center">Olá, eu sou o Raian Leite! 👋</h1>
  <h3>Desenvolvedor Back-end em formação | Entusiasta Java</h3>

  <p>
    🎓 Estudante de Ciência da Computação (8º período) na FUMEC<br>
    📍 Santa Luzia - MG | <b>Disponível para Remoto ou Presencial (Grande BH)</b><br>
    🚀 Transformando lógica e café em soluções de software eficientes.
  </p>

  <a href="https://www.linkedin.com/in/raian-leite/" target="_blank">
    <img src="https://img.shields.io/badge/-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank">
  </a>
  <a href="mailto:raianleite97@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/-Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" target="_blank">
  </a>
  <a href="https://www.instagram.com/raian_leite/" target="_blank">
    <img src="https://img.shields.io/badge/-Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" target="_blank">
  </a>
</div>

<br>

## 👨‍💻 Sobre Mim

Atualmente estou cursando o final da graduação em Ciência da Computação e focando meus estudos em **Engenharia de Software** e **Desenvolvimento Back-end**. Tenho experiência prévia com suporte de TI e infraestrutura, o que me deu uma base sólida para resolver problemas reais.

Hoje, meu foco principal é o ecossistema **Java**, estudando arquitetura de software, boas práticas (Clean Code) e construção de APIs.

---

## 🚀 Tech Stack

### ⚡ Principais (Foco Atual)
<div align="left">
  <img src="https://skillicons.dev/icons?i=java,spring,git,github,eclipse,postgres" height="40" alt="Main Stack" />
</div>

### 📚 Outros Conhecimentos & Ferramentas
<div align="left">
  <img src="https://skillicons.dev/icons?i=python,c,js,html,css,react,nodejs,vscode,arduino" height="40" alt="Other Skills" />
</div>

---

## 📌 Projetos em Destaque

| Projeto | Descrição | Techs |
| :--- | :--- | :--- |
| **[Console Task Manager](https://github.com/raian-leite/console-task-manager)** | Gerenciador de tarefas robusto com arquitetura em camadas (MVC), Streams API e persistência em memória. | `Java` `POO` `Clean Code` |
| **[CleanDesk](https://github.com/RaianLeite/CleanDesk)** | Automação em Python que organiza arquivos de downloads automaticamente por extensão. | `Python` `Automation` |

---

## 📊 Estatísticas do GitHub

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=RaianLeite&show_icons=true&count_private=true&theme=dracula&hide_border=true" height="160" alt="GitHub Stats" />
  
  <br>

  <img src="https://github-readme-stats.vercel.app/api/top-langs?username=RaianLeite&layout=compact&langs_count=8&theme=dracula&hide_border=true" height="140" alt="Top Langs" />

  <br><br>

  <img src="https://github-readme-activity-graph.vercel.app/graph?username=RaianLeite&theme=react-dark&area=true&hide_border=true" width="100%" alt="Activity Graph" />
</div>

---
<p align="center">
  <i>"A melhor maneira de prever o futuro é inventá-lo."</i><br>
  — <b>Alan Kay</b> (Pioneiro da Orientação a Objetos)
</p>
